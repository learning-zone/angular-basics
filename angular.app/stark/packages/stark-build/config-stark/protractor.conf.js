...
// first we try to locate the Selenium jar file
const seleniumFolder = helpers.root("node_modules/protractor/node_modules/webdriver-manager/selenium");
...
// Protractor will fail to load the e2e.spec.ts files due to "Unexpected token 'import'..."
...
// 2.- modify the TS config only for these E2E tests in order to generate CommonJS modules rather than ES2015 modules (we do this)
//
// so the TS config from the project (not the one from Stark!) is used but just changing the module type generation: CommonJS
...
const starkTsConfig = require(helpers.rootStark("./tsconfig.json"));
// IMPORTANT: also the "allowJs" option should be enabled so that the compiled JS code from Stark is transpiled as well
...
const compilerOptionsWithCommonJs = Object.assign({}, starkTsConfig.compilerOptions, projectTsConfig.compilerOptions, {
	module: "commonjs",
	allowJs: true
});

// Make sure that TS code can be loaded
// the compiler option "allowJs: true" is not enough, the code from Stark SHOULD be transpiled
// so the node_modules should be ignored EXCEPT those in @nationalbankbelgium
require("ts-node").register({
	compilerOptions: compilerOptionsWithCommonJs,
	ignore: [/node_modules(?!\/@nationalbankbelgium)/]
});
...
exports.config = {
	...
	// run.
	//suites: {
	//	smoke: "src/smoketests/**/*.e2e-spec.ts",
	//	full: "src/**/*.e2e-spec.ts"
	//},
